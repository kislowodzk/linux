...
configure_prompt() {
    # prompt_symbol="☭" # Dla alacritty
    prompt_symbol=" ☭ " # Dla kitty
    # ㉿
    ...
    case "$PROMPT_ALTERNATIVE" in
        twoline)
            ...
            ;;
        oneline)
            PROMPT=$'${debian_chroot:+($debian_chroot)}${VIRTUAL_ENV:+($(basename $VIRTUAL_ENV))}%B%F{%(#.red.blue)}%n@%m%b%F{reset}:%B%F{%(#.blue.yellow)}%~%b%F{reset}%(#.#.$) '
            RPROMPT=
            ;;
        backtrack)
